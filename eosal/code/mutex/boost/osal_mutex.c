...
****************************************************************************************************
  ...
  @brief Lock a mutex if mutex is unlocked, return immediately.
  @anchor osal_mutex_try_lock

  The osal_mutex_try_lock() function locks the mutex (increments mutex'es lock count), if mutex
  is unlocked. In this case the function behaves exactly like osal_mutex_lock(). If mutex
  is already locked, the function returns immediately OSAL_STATUS_MUTEX_ALREADY_LOCKED status.

  @param   mutex Mutex pointer returned by osal_mutex_create() function.
  @return  If mutex was successfully locked by this function, the function returns OSAL_SUCCESS(0).
		   Otherwise if the mutex was locked by an another thread, the function returns
		   OSAL_STATUS_MUTEX_ALREADY_LOCKED. Value OSAL_STATUS_FAILED can be returned if
		   the mutex given as argument is not pointer to valid mutex.
